  ...
  computed: {
    ...
    computedProducts: function () {
      ...
      return this.products.filter((item) => {
        return (this.search_product.trim().length === 0 || item.name.toLowerCase().includes(this.search_product.toLowerCase()))
        && (color.length === 0 || color.includes(item.colors[0])
        ...
        &&(size.length === 0 || size.includes(item.sizes[0]) || size.includes(item.sizes[1]) || size.includes(item.sizes[2]))
        &&(material.length === 0 || material.includes(item.materials[0]) || material.includes(item.materials[1]) || material.includes(item.materials[2]))
      })
    }
  },
  methods: {
